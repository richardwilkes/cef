...
type ClientProxy interface {
	...
	OnProcessMessageReceived(self *Client, browser *Browser, frame *Frame, sourceProcess ProcessID, message *ProcessMessage) int32
}
...
func (d *Client) OnProcessMessageReceived(browser *Browser, frame *Frame, sourceProcess ProcessID, message *ProcessMessage) int32 {
	return lookupClientProxy(d.Base()).OnProcessMessageReceived(d, browser, frame, sourceProcess, message)
}

//nolint:gocritic
//export gocef_client_on_process_message_received
func gocef_client_on_process_message_received(self *C.cef_client_t, browser *C.cef_browser_t, frame *C.cef_frame_t, sourceProcess C.cef_process_id_t, message *C.cef_process_message_t) C.int {
	me__ := (*Client)(self)
	proxy__ := lookupClientProxy(me__.Base())
